...
#'
quantile.r2.score = function(realized_estimates, forecast_values, quantile,
                             benchmark_values){
  ...
  for (ind in 1:length(forecast_values)){
    ...
    nom = nom + if_else(realized_error > 0, realized_error * quantile,
                        realized_error * (quantile - 1))

    denom = denom + if_else(benchmark_error > 0, benchmark_error * quantile,
