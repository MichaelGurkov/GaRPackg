...
# Transform variables to rates of change

df = df %>% 
  mutate_at(vars(vars_to_rates),.funs = list(~c(NA,diff(log(.)))))
...
# Add back one dimesions variables

reg_df = reg_df %>% 
  left_join(df %>% 
  select(Date, Tourists, Credit_to_non_fin_sector) %>% 
  mutate_at(vars(-Date), .funs = list(~scale(.))),
  by = "Date")
...
quant_reg_list = rq(formula = formula(GDP_F_1~.),tau = tau_vec,
              data = reg_df %>% select(-GDP_F, -GDP_F_4))

# quant_reg_list_4 = rq(formula = formula(GDP_F_4~.),tau = tau_vec,
#               data = reg_df)
...
\begin{tikzpicture}[scale=0.5, transform shape]
  ...
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	\node (non-energy-cmdty) [Item Style,  below =  \myvdist cm of glob-indust,
	fill = gray!40] {Commodity \\ (less energy)};
