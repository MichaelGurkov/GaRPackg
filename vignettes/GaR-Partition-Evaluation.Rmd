...
my_fig_height = length(pca_list) * 1.5

```


```{r plot_pca_variance, eval=FALSE}
...
plot_names = c("Domestic Macro","Global Conditions",
               "Financial Conditions \n (up = accomodating)",
               "Financial Cycle \n (credit, houseprice)")

pca_plots = lapply(seq_along(names(pca_list)), function(temp_ind){
  
  bar_plot = ggplot(data.frame(Name = names(pca_list[[temp_ind]]$pca$rotation[,1]),
                  Val = pca_list[[temp_ind]]$pca$rotation[,1]),
       aes(x = reorder(Name, Val), y = Val)) +
  geom_bar(stat = "identity", width = 0.5) +
  labs(x = "", y = "", title = plot_names[temp_ind]) +
  coord_flip() +
  theme_bw() +
  theme(plot.title = element_text(hjust = 0.5))
