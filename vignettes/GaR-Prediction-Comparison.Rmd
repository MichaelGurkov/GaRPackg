...
staff_forecast_df = df %>% 
  ...
  filter(complete.cases(.)) %>% 
  mutate(Time_Unit = str_remove(Forecast_Horizon,"\\d")) %>% 
  mutate(Forecast_Horizon = str_remove(Forecast_Horizon,"\\D")) %>% 
  mutate(Forecast_Horizon = if_else(Time_Unit == "Q",as.numeric(Forecast_Horizon) / 4,
                                    as.numeric(Forecast_Horizon))) %>% 
  select(-Time_Unit) %>% 
  mutate(New_Date = Date + Forecast_Horizon)
