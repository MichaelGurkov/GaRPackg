...
models_list = list(GaR_Forecast = partitions_list$basic,
...
```{r basic_forecast, eval=FALSE}

forecast_data_list$Basic_Forecast = get.gar.forecast(
  partitions_list = partitions_list$two_features,
  vars_df = forecast_parameters$df,
  target_var_name = "GDP",
  horizon_list = parameters_list$horizon_list,
  quantile_vec = parameters_list$quantile_vec,
...
gar_basic_r2_df = forecast_data_list[c("Basic_Forecast",
                                       "Intercept_Forecast")] %>% 
  reduce(inner_join,
  ...
  rename(Basic_Forecast = GaR_forecast)

forecast_model_fit$gar_basic_r2 = gar_basic_r2_df %>% 
  group_by(Horizon, Quantile) %>% 
  summarise(
    gar_r2_score = quantile.r2.score(
    realized_estimates = GDP_actual,
    forecast_values = Basic_Forecast,
    quantile = as.numeric(Quantile[1]),
    benchmark_values = Intercept_Forecast))

rm(gar_basic_r2_df)

ggplot(forecast_model_fit$gar_basic_r2,
       aes(x = Quantile, y = gar_r2_score)) + 
  geom_col(width = 0.3) + 
  facet_wrap(~Horizon) + 
  labs(x = "", y = "", title = "Goodness of fit of Basic (GDP + Credit) model")
