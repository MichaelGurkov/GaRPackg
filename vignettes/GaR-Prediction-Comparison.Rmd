...
temp_gar = forecast_data_list[c("gar_forecast","intercept_forecast")] %>% 
...
temp_macro = forecast_data_list[c("macro_gar_forecast","intercept_forecast")] %>% 
  reduce(inner_join, by = c("Quantile","Horizon","Forecast_Status",
                            "Win_Type","Forecast_Period","Date"),
         suffix = c("_model","_benchmark")) %>% 
  left_join(forecast_data_list$actual_gdp_df, by = "Forecast_Period")
...
ggplot(forecast_model_fit$macro_actual_fit_score %>% 
  ...
  facet_grid(rows = vars(Forecast_Status), cols = vars(Win_Type), scales = "free_y") + 
  geom_col(width = 0.5) + 
  labs(x = "", y = "", title = "Macro spec - quantile fit score") + 
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1)) + 
  theme_bw() + 
  theme(legend.position = "bottom", plot.title = element_text(hjust = 0.5))
...
```{r gar_vs_macro_quantile_area_fit, eval=FALSE}
