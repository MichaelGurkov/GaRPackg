...
temp_gar_forecasts_list = map(names(models_list), function(temp_name){
  ...
  mutate(Forecast_Period = paste(
    ...
    select(Date, Horizon, Quantile,Forecast_Period,Forecast) %>% 
    rename(!!sym(temp_name) := Forecast)
...
names(temp_gar_forecasts_list) = names(models_list)

forecast_data_list = c(forecast_data_list, temp_gar_forecasts_list)

rm(models_list, temp_gar_forecasts_list)
...
gar_macro_r2_df = map(c("gar","intercept","macro"), function(temp_name){
  
  temp_df = forecast_data_list[[temp_name]] %>% 
...
forecast_model_fit$gar_macro_r2 = gar_macro_r2_df %>% 
  ...
  summarise(
    ...
    benchmark_values = intercept),
    macro_r2_score = quantile.r2.score(
    realized_estimates = GDP_actual,
    forecast_values = macro,
    quantile = as.numeric(Quantile[1]),
    benchmark_values = intercept))
