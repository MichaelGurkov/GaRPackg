...
forecast_data_list$intercept_forecast = lapply(c("fixed","expanding"),
                         function(temp_win_type){
...
The first stage is to obtain in sample and out of sample forecast. The in sample forecast is just the fitted values, the out of sample forecast is calculated in the following manner : first a model is fitted on a subset of the data (the window) and then the outcome is predicted based on new data that is determined by the out of sample step (currently next period)
...
forecast_comparisons$gar_macro = list(
...
The third stage is model fit evaluation which is performed according to several metrics:
RMSE, quantile fit score and quantile fit area score.

```{r rmse, eval=FALSE}

forecast_model_fit$gar_actual_rmse =  
  forecast_comparisons$gar_actual %>% 
  group_by(Horizon,Forecast_Status,Win_Type) %>% 
  summarise(RMSE = sqrt(mean(Error ^ 2)), .groups = "drop")


forecast_model_fit$macro_rmse =
  forecast_comparisons$gar_macro %>% 
  group_by(Horizon,Forecast_Status,Win_Type) %>% 
  summarise(RMSE = sqrt(mean(GaR_forecast_macro_Error ^ 2)),
            .groups = "drop")
...
```{r quantile_fit_score, eval=FALSE}

forecast_periods = 
  unique(forecast_data_list$gar_forecast$Forecast_Period)[
    unique(forecast_data_list$gar_forecast$Forecast_Period) %in%
      forecast_data_list$actual_gdp_df$Forecast_Period]


forecast_model_fit$gar_actual_fit_score = 
  lapply(forecast_periods,
...
forecast_model_fit$macro_actual_fit_score = 
  lapply(forecast_periods,
         function(temp_period){
...
forecast_model_fit$gar_actual_fit_crps = 
  lapply(forecast_periods,
         function(temp_period){
  ...
  temp_df = forecast_data_list$gar_forecast %>%
    filter(Forecast_Period == temp_period) %>% 
    group_by(Horizon, Win_Type,Forecast_Status) %>% 
    summarise(Score = quantile.crps.score(
      realized_estimate = actual_gdp,
      quantile_values = GaR_forecast,
      quantiles = as.numeric(Quantile)),
      .groups = "drop") %>% 
    ungroup() %>% 
...
forecast_model_fit$macro_actual_fit_crps = 
  lapply(forecast_periods,
         function(temp_period){
...
temp_gar = forecast_data_list[c("gar_forecast",
                                "intercept_forecast")] %>% 
  reduce(inner_join, by = c("Quantile","Horizon","Forecast_Status",
                            "Win_Type","Forecast_Period","Date"),
         suffix = c("_model","_benchmark")) %>% 
  left_join(forecast_data_list$actual_gdp_df, by = "Forecast_Period")

temp_macro = forecast_data_list[c("macro_gar_forecast",
                                  "intercept_forecast")] %>% 
  reduce(inner_join, by = c("Quantile","Horizon","Forecast_Status",
                            "Win_Type","Forecast_Period","Date"),
         suffix = c("_model","_benchmark")) %>% 
...
forecast_model_fit$gar_R2 = temp_gar %>% 
  filter(complete.cases(.)) %>% 
  group_by(Quantile, Horizon, Forecast_Status,Win_Type) %>% 
  summarise(Score = quantile.r2.score(
    realized_estimates = GDP_actual,
    forecast_values = GaR_forecast_model,
    quantile = as.numeric(Quantile[1]),
    benchmark_values = GaR_forecast_benchmark),
    .groups = "drop")


forecast_model_fit$macro_R2 = temp_macro %>% 
  filter(complete.cases(.)) %>% 
  group_by(Quantile, Horizon, Forecast_Status,Win_Type) %>% 
  summarise(Score = quantile.r2.score(realized_estimates = GDP_actual,
                                      forecast_values = GaR_forecast_model,
                                      quantile = as.numeric(Quantile[1]),
                                      benchmark_values = GaR_forecast_benchmark),
            .groups = "drop")
...
```{r plot_macro_quantile_fit_score, eval=FALSE}

ggplot(forecast_model_fit$macro_actual_ %>% 
  group_by(Horizon,Win_Type,Forecast_Status) %>% 
  summarise(SD = mean(Score, na.rm = TRUE), .groups = "drop"),
  aes(x = Horizon, y = SD, fill = Horizon)) + 
  facet_grid(rows = vars(Forecast_Status), cols = vars(Win_Type),
             scales = "free_y") + 
  geom_col(width = 0.5) + 
  labs(x = "", y = "",
       title = "Macro spec - average quantile fit score") + 
  scale_y_continuous(
...
ggplot(forecast_model_fit$macro_R2,
       aes(x = Quantile, y = Score, fill = Horizon)) + 
  geom_col(position = "dodge", width = 0.5) + 
  facet_grid(vars(Forecast_Status), vars(Win_Type)) + 
...
\subsection{GaR vs Intercept}

```{r plot_gar_R2}

ggplot(forecast_model_fit$gar_R2,
       aes(x = Quantile, y = Score, fill = Horizon)) + 
  geom_col(position = "dodge", width = 0.5) + 
  facet_grid(vars(Forecast_Status), vars(Win_Type)) + 
  scale_fill_viridis_d(option = "plasma") + 
...
```{r gar_vs_macro_rmse, eval=FALSE}
...
ggplot(forecast_model_fit[c("gar_actual_rmse","macro_rmse")] %>% 
  ...
  labs(x = "", y = "", title = "GaR vs Macro only - RMSE") + 
  theme_bw() + 
  theme(legend.position = "bottom",
        plot.title = element_text(hjust = 0.5),
        legend.title = element_blank())
...
```{r gar_vs_macro_quantile_fit, eval=FALSE}
...
ggplot(forecast_model_fit[c("gar_actual_fit_crps",
                            "macro_actual_fit_crps")] %>% 
         reduce(inner_join,
                by = c("Horizon", "Forecast_Status","Win_Type",
                       "Forecast_Period"),
                suffix = c("_gar","_macro")) %>% 
         select(-starts_with("Forecast_Period")) %>% 
         filter(Forecast_Status == "Out of Sample") %>% 
       ...
       aes(x = Model, y = value, fill = Model)) + 
  geom_col(position = "dodge", width = 0.5) + 
  scale_fill_viridis_d(labels = c("GaR", "Macro only")) + 
  # scale_fill_hue(labels = c("GaR", "Macro only")) + 
  facet_grid(rows = vars(Win_Type), cols = vars(Horizon),
             scales = "free_y") + 
  labs(x = "", y = "",
       title = "GaR vs Macro only  - CRPS") + 
  theme_bw() + 
  theme(legend.position = "bottom",
        plot.title = element_text(hjust = 0.5),
        legend.title = element_blank())
...
temp_df = list(forecast_model_fit$gar_R2 %>% 
       filter(Forecast_Status == "Out of Sample",
              Win_Type == "fixed") %>% 
       select(-Forecast_Status,-Win_Type),
     forecast_model_fit$macro_R2 %>% 
       filter(Forecast_Status == "Out of Sample", Win_Type == "fixed") %>% 
       select(-Forecast_Status,-Win_Type)) %>% 
       reduce(inner_join, by = c("Quantile", "Horizon"),
              suffix = c("_GaR", "_Macro_only")) %>% 
       pivot_longer(cols = starts_with("Score"), names_to = "Model",
                    values_to = "Score") %>% 
       mutate(Model = str_remove_all(Model,"Score_"))
