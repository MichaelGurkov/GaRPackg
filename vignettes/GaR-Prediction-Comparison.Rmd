...
forecast_data_list$gar_forecast = get.gar.forecast(
  ...
  mutate(Forecast_Period = paste(
    Forecast_Date + as.numeric(Horizon) * 0.25 - 1,
    Forecast_Date + as.numeric(Horizon) * 0.25,
    sep = "-")) %>% 
  mutate(Horizon = as_factor(Horizon)) %>% 
  select(Forecast_Date, Horizon, Quantile,Forecast_Status,
...
macro_gar_model = run.GaR.analysis(partitions_list = partitions_list$macro,
                   vars_df = df,
                   target_var_name = "GDP",
                   horizon_list = parameters_list$horizon_list,
                   quantile_vec = parameters_list$quantile_vec,
