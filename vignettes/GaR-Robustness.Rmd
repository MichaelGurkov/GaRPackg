...
match_partition_from_id = function(temp_id, categories_list){
  ...
  get_vars_from_category = function(category_string, id_str){
  
   temp_res = unlist(map(category_string, function(temp_str){
...
match_partition_from_id(temp_id = best_models$id[1],categories_list = categories_list)


temp_q_2 = best_models %>% 
  select(id) %>% 
  mutate(partition = map(id,
                         ~match_partition_from_id(temp_id = .,
                                                  categories_list = categories_list))) %>% 
  mutate(forecast = map(partition,
                        ~get_gar_forecast(partitions_list = .,
                                          vars_df = raw_df,
                                          target_var_name = "gdp_yoy",
                                          horizon_list = c(12),
                                          quantile_vec = c(0.1,0.25,0.5,0.75,0.95)))) %>% 
  mutate(quantile_r2 = map(forecast, ~quantile_r2_score(forecast_df = .,
