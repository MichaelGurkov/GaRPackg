...
fit_df %>% 
  ...
  pivot_wider(id_cols = -Horizon,
              names_from = Horizon,
              values_from = Coeff,
              names_prefix = "h_") %>% 
  mutate(across(c("h_1","h_4","h_8","h_12"), ~ . - h_1)) %>% 
  pivot_longer(cols = c(-rowname,-Tau),
               names_to = "Horizon",
               values_to = "NormCoeff") %>% 
  filter(!Horizon == "h_1") %>% 
  mutate(Horizon = str_remove_all(Horizon,"h_")) %>% 
  mutate(Horizon = factor(Horizon,c(4,8,12))) %>% 
  ggplot(aes(x = NormCoeff, fill = Tau)) + 
  geom_histogram() + 
  geom_vline(xintercept = 0, linetype = "dashed") + 
  ylab(NULL) + xlab(NULL) +  ggtitle(" Normalized coefficients") + 
