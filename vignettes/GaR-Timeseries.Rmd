...
gar_factors_df = results$reg_df %>% 
  ...
  as.matrix() * coefficients(results$quantile_reg$`1`)[-1,1]

gar_factors_df = cbind(Date = results$reg_df$Date, as.data.frame(gar_factors_df))
...
plot_df = gar_factors_df %>% 
  gather(key = Partition, value = GaR, -Date)
...
ggplot() + 
  ...
  labs(title = "Growth at risk partition decomposition",
       x = "", y = "") + 
  theme_bw() + 
