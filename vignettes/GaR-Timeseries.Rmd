...
prediction_df %>% 
  ...
  ggplot(aes(x = date, y = value, color = quantile)) + 
  geom_line() + 
...
fan_chart_df  = prediction_df %>% 
  ...
  rename(Horizon = horizon, Quantile = quantile, GaR = value) %>% 
  fix_quantile_crossing() %>% 
  rename_all(tolower) %>% 
  pivot_wider(names_from = quantile,values_from = gar) %>% 
