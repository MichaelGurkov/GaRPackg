...
In-sample properties of the distribution of expected inflation:
  \begin{enumerate}
	...
	\item
	Oil prices and the output gap contribute to the	evolution of forecast uncertainty,
...
\begin{itemize}
  ...
  in month $t$
  \item
  $\pi^{e}_{t}$ - is the monthly average of five-year five-year forward breakeven
  in inflation rates.
  \item
...
```{r plot_factor_decomposition, fig.height=6, eval=FALSE}
...
factor_plot_data = iar_analysis %>%
  ...
  filter(horizon == 12) %>%
  select(-horizon) %>% 
  mutate(exp_int = intercept + beir5y5y) %>% 
  ...
  mutate(name = factor(name,
                       levels = c("cpi","exp_int","nis_usd","brent_usd","meshulav_gap"),
                       labels = c("Inflation","Expectations","Exchange Rate",
