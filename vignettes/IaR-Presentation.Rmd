...
"the Committee members assessed that at the current time, there are \textbf{risks of inflation} remaining around the upper part of the target for a longer time."

\vspace{1em}

Report of BoI monetary policy discussions, Feb 2022
...
* Based on Banerjee et al. (2020), Lopez-Salido, J. D. and F. Loria (2020)
...
In-sample properties of the distribution of expected inflation:
  ...
  \setlength\itemsep{2em}
	...
	\item
	Realized inflation and long-term expectations are the predominant
	factors explaining	these changes
	\item
	Oil prices contribute to forecast uncertainty,
	declining inflation environment explains variation	in skewness.
  \end{enumerate}
...
$\tau$ 12 months ahead (sample: 2004-2019, monthly frequency).
...
skew_df%>%
  ...
  labs(x = "", y = "",
       caption = paste0("Note: the dashed lines represent",
                        ...
                        " Blue lines are sample averages.")) +
...
<!-- ## Realized inflation increases upside risks and uncertainty -->
...
<!-- ## Inflation expectations increase upside risks and lower uncertainty -->
...
<!-- ## Oil prices stimulate inflation and increase uncertainty and skeweness -->
...
## Policy rate and the ELB period
...
df %>% 
  ...
  ggplot(aes(date, boi_rate)) + 
  geom_line() + 
  annotate("rect", xmin = as.yearmon("Sep 2014"),
           xmax = as.yearmon("Dec 2019"),
           ymin=0, ymax=Inf,
           alpha = 0.5, fill="lightblue") + 
  geom_text(aes(x = as.yearmon("Jul 2017"), y = 4.5),
            label = "Effective Lower Bound \n period",
            family = "mono", size = 6) + 
  xlab(NULL) + ylab(NULL) + ggtitle("Bank of Israel policy rate")

```

## The Effective Lower Bound may have played a role

\label{FactorPlotELB}


```{r plot_skew_contributions_elb,fig.height=7}

factor_contributions_elb = calculate_disp_and_skew_contributons(iar_analysis_elb)

factor_contributions_elb = list("contribution_iqr" = factor_contributions_elb %>% 
  select(date, horizon, category, contribution_iqr) %>% 
  mutate(indicator = if_else(str_detect(category,"_elb$"), "base","elb")) %>% 
  mutate(category = str_remove_all(category,"_elb")) %>% 
  pivot_wider(names_from = indicator,values_from = contribution_iqr) %>% 
  mutate(contribution_iqr = base + elb) %>% 
  select(-c("base","elb")),
  "contribution_skew" = factor_contributions_elb %>% 
  select(date, horizon, category, contribution_skew) %>% 
  mutate(indicator = if_else(str_detect(category,"_elb$"), "base","elb")) %>% 
  mutate(category = str_remove_all(category,"_elb")) %>% 
  pivot_wider(names_from = indicator,values_from = contribution_skew) %>% 
  mutate(contribution_skew = base + elb) %>% 
  select(-c("base","elb")),
  "skew_iqr" = factor_contributions_elb %>% 
    select(date,horizon,category,skew,iqr)) %>% 
  reduce(inner_join, by = c("date","horizon","category"))

factor_contributions_elb = factor_contributions_elb %>%
    group_by(date = as.yearqtr(date), category, horizon) %>%
    summarise(across(everything(), mean, na.rm = TRUE),
              .groups = "drop")

plot_df_elb = make_plot_df(factor_contributions_elb,labels_df)
...
rm(factor_contributions_elb, plot_df) 
...
ggplot() +
  geom_ribbon(
    data = iar_forecast %>%
      filter(year(forecast_target_date ) <= 2019) %>% 
      filter(horizon == 12) %>%
...
\setlength\itemsep{2em}
	...
	2004 - 2015 and adds one observation in each step)
\end{itemize}
...
For each quantile $\tau$ we compute  the percentage of observations that fall below the forecast quantile $\hat{Q}^{\tau}_{t}$:
	$$
	\varphi_{\tau} \equiv \frac{1}{T} \sum_{t=1}^{T} \mathbb{I}_{ \left\{ \pi_{t} <  \hat{Q}^{\tau}_{t} \right\} }.
	$$
\begin{itemize}
\setlength\itemsep{1em}
	\item
...
OS_size=iar_forecast %>% 
  filter(horizon == 12) %>% 
  inner_join(select(df, date, cpi_yoy),
             by = c("forecast_target_date" = "date")) %>% 
...
list(iar_pit, intercept_pit, inflation_pit) %>%
  ...
  xlim(0,1) + ylim(0,1) +
  xlab("Quantile") + ylab(NULL) + 
  labs(caption = paste0("Dashed lines are confidence bands",
                        " (Rossi et al 2019) at p = 0.05 ")) + 
...
$$
R^2_{\tau} \equiv 1- \frac{\sum_{t} \left( y_{t} - \hat{Q}^{\tau}_{t} \right) \left(\tau - \mathbb{I}_{ \left\{ y_{t} - \hat{Q}^{\tau}_{t}>0 \right\} } \right) }{ \sum_{t} \left(y_{t} - \hat{c}^{\tau}_{t} \right) \left(\tau - \mathbb{I}_{ \left\{ y_{t} - \hat{Q}^{\tau}_{t}>0 \right\} } \right) }
$$

$\hat{Q}^{\tau}_{t}$  -- model's forecasts for quantile
$\tau$

$\hat{c}^{\tau}_{t}$ -- intercept only forecast
...
\setlength\itemsep{3em}
  ...
  in these developments. The ELB might have also played an important role.
  \item
...
## Output gap depresses inflation while increasing upside risks one year ahead{.unlisted .unnumbered}
...
## Exchange rate increases downside risks and lowers uncertainty one year ahead{.unlisted .unnumbered}
...
## Quantile Regression{.unlisted .unnumbered}
...
## Autoregression in Output Gap{.unlisted .unnumbered}
...
## Positive (short term) Output Gap{.unlisted .unnumbered}
...
iar_analysis %>%
  ...
  scale_fill_manual(values = c("significant" = "lightblue",
                    ...
                    labels = c("Significant", "Non significant")) +
  geom_hline(yintercept = 0, color = "black") + 
  xlab("Quantile") + ylab(NULL) + ggtitle("Output Gap coefficients at 1 month ahead") +
  labs(
    caption = paste0(
      "Notes: Each column shows the estimated coefficients ",
      "from the quantile regression. ",
...
## Output Gap{.unlisted .unnumbered}
...
## Out of sample inflation only{.unlisted .unnumbered}
...
## Out of sample intercept only{.unlisted .unnumbered}
