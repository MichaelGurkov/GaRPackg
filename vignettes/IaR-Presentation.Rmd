...
\begin{itemize}
  \item Lopez-Salido and Loria (2019) - United States and Euro area predictive inflation
  distribution. Conditional mean of inflation does not convey an adequate representation
  of the overall pattern of inflation dynamics. Changes in credit conditions are also key
  to understand the dynamics of the inflation tails.
\end{itemize}
...
## Ralized inflation increases upside risks and uncertainty
...
iqr_df %>% 
  filter(horizon == 12) %>% 
  left_join(select(df, c(date, cpi))) %>% 
  select(-date) %>% 
  pivot_longer(cols = c(skew,iqr)) %>% 
...
## Output gap increases upside risks
...
iqr_df %>% 
  filter(horizon == 12) %>% 
  left_join(select(df, c(date, x = meshulav_gap))) %>% 
  select(-date) %>% 
  pivot_longer(cols = c(skew,iqr)) %>% 
  ...
  geom_smooth(method = "lm") + 
  facet_wrap(~name, scales = "free") + 
  xlab("Ouput gap") + ylab(NULL)

```


## Oil price increases downside risks
...
## Exchange rate increases downside risks and uncertainty
...
## Higher Median Forecasts are Associated with Higher Uncertainty
