...
temp_df = df %>%
  select(cpi,beir5y5y,brent_usd, meshulav_gap) %>%
  mutate(across(everything(), ~lead(.,12),.names = "{.col}_12")) %>%
  filter(complete.cases(.))


temp_qreq = map_dfr(c("cpi","beir5y5y","brent_usd","meshulav_gap"),
                    function(temp_var){
  
  temp_qreq = rq(formula = formula(glue("cpi_12 ~ ",temp_var)),
               tau = c(0.05,0.5,0.95),
               data = temp_df) %>%
  coefficients() %>%
  t() %>%
...
ggplot() +
  geom_point(data = temp_df,
             aes(x = cpi, y = cpi_12)) +
  geom_abline(data =  filter(temp_qreq,feature == "cpi"),
              aes(intercept = int,
                  slope = slope,
                  color = quantile)) +
