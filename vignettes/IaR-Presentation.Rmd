...
\itemsep3em
  \item
  A risk management approach to monetary policy requires the distribution around most
  likely inflation path
	\item
	Analyzing the distribution of future inflation since the current inflation target
	range was set.
	\item
...
In-sample properties of the distribution of expected inflation:
  \begin{enumerate}
	\item
	Balanced risk and stable uncertainty before 2014, decrease in uncertainty
...
$\tau$	and horizon 12 months ahead:

$Q^{\tau}_{t,h}=\beta^{\tau}_{h}X_t+\epsilon^{\tau}_{t,h}$

$X_t=[1, \pi_{t}, \hat{y}_{t}, oil_{t}, s_{t}]$
...
\begin{itemize}
  \item
  $\pi_{t}$ is the YoY CPI inflation excluding fruits, vegetables, and housing
  in month $t$
  \item
  $\hat{y}_{t}$ is a monthly estimate of the output gap (HP filtered
  State-of-the Economy Index).
  \item
  $oil_{t}$ - is the monthly percentage change in Brent Crude oil prices,
  denominated in USD.
  \item
  $s_{t}$ - is the monthly percentage change in the NIS-USD exchange rate.
\end{itemize}
...
* The sample period is 2004-2019 at monthly frequency

* We focus on 12 months ahead horizon
...
coeff_df = iar_analysis %>%
  ...
  mutate(partition = factor(
    partition,
    levels = c("cpi", "meshulav_gap","brent_usd","nis_usd"),
    labels = c("Inflation","Output Gap", "Oil prices","Exchange Rate")
  ))
...
band_df = tibble(name = rep("Skewness",2), val = c(skew_se, -skew_se))

skew_df%>%
  ggplot(aes(x = date, y = value)) +
  geom_line() +
  facet_wrap(~name, scales = "free") +
  geom_hline(aes(yintercept = val), color = "blue", data = mean_df) + 
  geom_hline(aes(yintercept = val), color = "black", data = band_df,
              linetype = "dashed") +
 ...
 rm(skew_df, mean_df, band_df, skew_se, n)

```

$Dispersion_{t} \equiv Q^{0.75}_{t} - Q^{0.25}_{t} \quad Skeweness_{t} \equiv \frac{Q^{0.75}_{t} + Q^{0.25}_{t} -2Q^{0.5}_{t}}{Dispersion_{t}}$
...
factor_plot_data = iar_analysis %>%
  ...
  mutate(name = factor(name,
                       ...
                       labels = c("Inflation","Exchange Rate","Oil prices",
                                  "Output Gap"))) %>% 
  select(date, name, dispersion, skewness) %>% 
  group_by(date = as.yearqtr(date), name) %>% 
  summarise(across(c(dispersion, skewness),mean), .groups = "drop")


disp_plot = ggplot() +
  geom_col(data = factor_plot_data,  aes(x = date, y = dispersion, fill = name)) +
  geom_line(
    data = factor_plot_data %>%
      group_by(date) %>%
      summarise(total_disp = sum(dispersion), .groups = "drop"),
    aes(x = date, y = total_disp), size = 1
  ) +
  xlab(NULL) + ylab(NULL) + ggtitle("Dispertion")

skew_plot = ggplot() +
  geom_col(data = factor_plot_data,  aes(x = date, y = skewness, fill = name)) +
  geom_line(
    data = factor_plot_data %>%
      group_by(date) %>%
      summarise(total_skew = sum(skewness), .groups = "drop"),
    aes(x = date, y = total_skew), size = 1
  ) +
...
$E(Dispersion_{t + 12}|X_{t}) = (\beta^{0.75} - \beta^{0.25})X_{t} \quad E(Skeweness_{t + 12}|X_{t}) = \frac{(\beta^{0.75}_{t} + \beta^{0.25}_{t} -2\beta^{0.5}_{t})X_{t}}{Dispersion_{t+12}}$
...
## Output gap increases uncertainty and depresses inflation one year ahead
...
## Oil price stimulate inflation and decreases uncertainty one year ahead
