...
temp_df = df %>%
  select(cpi,beir5y5y) %>%
  add_leads_to_target_var("cpi",leads_vector = 12) %>%
  filter(complete.cases(.))


temp_qreq = rq(formula = formula("cpi_12 ~ beir5y5y"),
               tau = c(0.05,0.5,0.95),
               data = temp_df) %>%
  coefficients() %>%
  t() %>%
...
```{r plot_factor_decomposition, fig.height=7}
...
# 
# ggdraw(add_sub(plots_row, "Deviations from the mean"))
