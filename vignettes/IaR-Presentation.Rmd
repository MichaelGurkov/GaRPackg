...
df = df %>% 
  mutate(meshulav_gap = filter_obj$cycle / filter_obj$trend * 100)
...
iar_forecast_smoothed = iar_forecast %>% 
  fit_t_skew_to_gar_df(time_limit = 150,return_smoothed_quantiles = TRUE)
...
\itemsep3em
	\item
	Assessing risks to future inflation is essential for making knowledgeable
	forecasts.
	\item
...
* Based on Adrian et al. (2019), operationalized by the IMF (Prasad et al., 2019).
...
iar_analysis %>%
  ...
  # scale_fill_discrete(labels = c("Significant", "Non significant")) +
...
# iar_r2 = quantile_r2_score(
#   forecast_df = iar_forecast_smoothed %>% 
#     mutate(quantile = as.character(quantile)) %>% 
#     select(date, quantile, horizon, forecast_values = value),
#   actual_df = select(df, actual_values = cpi, date),
#   benchmark_df = intercept_forecast %>%
#     rename(benchmark_values = forecast_values),
#   frequency = "monthly"
