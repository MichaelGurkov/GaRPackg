...
df = df %>% 
  inner_join(raw_data$raw_df %>% 
               select(Date, Avg_House_Price_index,Credit_to_non_fin_sector,BOI_rate) %>% 
               rename(House = Avg_House_Price_index),
             by = "Date")
...
processed_df = df %>% 
  ...
  mutate(Credit_to_GDP = GDP_Nominal / Credit_to_non_fin_sector) %>% 
