...
@Adrian2019 is a seminal paper that lays the foundation of the GaR approach. The authors model the full distribution of GDP growth as a function of current economic and financial conditions. The empirical specification is quite limited : economic conditions are represented by GDP growth and financial conditions are represented by National Financial Conditions Index (NFCI). The NFCI 
provides a weekly estimate of US financial conditions in money markets, debt and equity markets, and the traditional and shadow banking systems. The empirical analysis involves estimating conditional quantiles (using quantile regression) and fitting a skewed t distribution.
The sample period spans 1973Q1-2018Q4 and the two forecast horizons are a quarter and a year ahead. The authors report a negative correlation between the median and the interquartile range of the future growth distribution. A comparison of conditioning on past GDP growth only vs conditioning on both GDP and financial conditions reveals that during business cycle
expansion financial conditions result lower variance and positive skewness relative to conditioning on GDP only. However, during business cycle contractions financial conditions induce greater variance, negative skewness and lower mean. 
...
FCI or simpler indices such as spreads (for example sovereign spread). The authors note
that market based variables are mostly pronounced in the short term while credit based variables perform better well over one year (or more) horizon.Another important finding is the positive contribution (at one to two years ahead) of credit gap that increases over the forecasting horizon. The authors also note that the forecasts are quite volatile and the use of the model is challenging.
...
policy perspective. The analysis reveals that at the medium term "excessive" credit
...
these variables predict financial (banking) crises. As for the financial conditions effect,
the results reveal that tightening in financial conditions depress growth in the 
