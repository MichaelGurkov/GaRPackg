---
title: "Inflation Risks in Israel - with CPI (excl_fv_hous)"
...
iar_analysis = run_GaR_analysis(
  ...
  vars_df = df,
  target_var_name = "cpi_excl_fv_hous",
...
ols_coeffs = iar_analysis$reg_df %>%
  select(cpi_excl_fv_hous_12, ends_with("xreg")) %>%
  lm(paste("cpi_excl_fv_hous_12 ~ ."), .) %>%
  tidy() %>%
  filter(!term == "(Intercept)") %>%
  mutate(term = str_remove_all(term, "_xreg")) %>%
  mutate(partition = factor(
    term,
    levels = c("cpi_excl_fv_hous", "beir5y5y" , "meshulav_gap", "brent_usd", "nis_usd"),
    labels = c(
      "Inflation",
      "Expectations",
      "Output Gap",
...
inflation_forecast = get_gar_forecast(
  partitions_list = list(inflation = "cpi_excl_fv_hous"),
  vars_df = df,
  target_var_name = "cpi_excl_fv_hous",
  horizon_list = horizon_list,
  quantile_vec = quantile_vec,
