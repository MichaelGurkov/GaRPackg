...
partitions = list(domestic_macro = list('real_gdp_yoy',
                  ...
                  world_fci = list('nasdaq_returns_qoq','vix', 'nasdaq_vol',
...
partitions_df = lapply(partitions,function(temp_part){
    ...
    chain_index(sign_align_params = temp_part[[1]])}) %>% 
...
invisible(sapply(names(qregs_results), function(temp_horizon){
  
  coef_table = extract.qreg.coeff.table(qregs_results[[temp_horizon]])
