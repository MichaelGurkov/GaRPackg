...
```{r make_synthetic_df, eval=FALSE}
...
new_df = inner_join(
  x = select(df, c("date",df_vars)),
  y = select(fame_data, c("date",fame_vars)),
  by = "date")
...
compare_df = inner_join(
  x = roman_data %>% 
    ...
    mutate(sovereigh_spread = sovereigh_spread * 0.01) %>% 
    mutate(avg_cpi = avg_cpi / lag(avg_cpi) - 1) %>% 
    pivot_longer(-date),
  y = new_df %>% 
    pivot_longer(-date),
  by = c("date","name"),
  suffix = c("_roman","_osnat")
  ) %>% 
  filter(complete.cases(.)) %>% 
  mutate(percent_diff = (value_roman/value_osnat) - 1)
...
correl_df = compare_df %>% 
  group_by(name) %>% 
  summarise(corr = cor(value_roman,value_osnat,
...
compare_df %>% 
  filter(str_detect(name,"volume_gov_bond")) %>% 
