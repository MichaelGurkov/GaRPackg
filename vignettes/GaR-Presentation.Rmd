...
plot_list = map(list("GDP","Credit"), function(temp_var){
  ...
  temp_df = df %>%
  select(all_of(temp_var)) %>%
  add_leads_to_target_var(target_var_name = temp_var,
                          leads_vector = 4) %>%
  filter(complete.cases(.))


temp_qreq = rq(paste0(temp_var,"_4","~",temp_var),
               tau = c(0.05,0.5,0.95),
               data = temp_df) %>%
  coefficients() %>%
  t() %>%
...
temp_plot = temp_df %>%
  ...
  ylab((paste("YoY", temp_var,
              "growth 4 quarters ahead"))) +
  ggtitle(temp_var) + 
  theme_bw() + 
