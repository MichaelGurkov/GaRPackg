...
factors_df = extract_factor_contribution_from_gar_model(results) %>% 
  mutate(horizon = factor(horizon, levels = c(1,4,8,12)))

factors_df = factors_df %>% 
  rename(`Dom. Fin. Conditions` = Dom_Fin) %>% 
  rename(`Dom. Macro` = Dom_Macro) %>% 
  rename(`Ext. Macro` = Ext_Macro) %>%
  rename(`Ext. Fin. Conditions` = Ext_Fin) %>%
  rename(`Dom. Fin. Cycle` = Dom_Fin_Cycle) %>% 
...
coeffs_df %>% 
  filter(name %in% c("Dom. Fin. Cycle")) %>% 
  filter(horizon %in% c(1,12)) %>% 
  ggplot(aes(x = quantile, y = coeff, fill = significant)) + 
  scale_fill_manual(values = c("Significant" = "lightblue",
                               "Non Significant" = "lightgrey")) + 
