...
factors_df %>%
  ...
  facet_wrap(~Horizon,labeller = labeller(Horizon = my_lab)) +
  xlab(NULL) + ylab(NULL) +
  ggtitle(paste0("Contribution of Factors to the",
...
In contrary to other countries, \textbf{no evidence of a skewed distribution}:
...
results %>% 
  ...
  filter(Horizon == 1) %>% 
  select(Date, Skew) %>% 
>>>>>>> 08649d8fe76a18d0c0e3583f9c9ab94fc18437e4
...
```{r plot_skew_robustness, cache=TRUE}
...
=======
  geom_hline(yintercept = 0, linetype = "dashed",
             color = "black") + 
  xlab(NULL) + ylab(NULL) + 
  ggtitle("Quartile Skewness 1 quarter ahead") + 
  theme_bw()
...
df %>% 
  ...
  rename(US = gdp_us, Euro = gdp_us, Israel = GDP) %>% 
  pivot
