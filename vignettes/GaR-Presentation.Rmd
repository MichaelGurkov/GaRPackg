...
plot_params$title_size = 30

plot_params$axis_labels_size = 20

plot_params$axis_text_size = 15
...
temp_plot = temp_df %>%
  ...
  theme(plot.title = element_text(size = plot_params$title_size),
        axis.title = element_text(size = plot_params$axis_labels_size),
        axis.text = element_text(size = plot_params$axis_text_size),
        legend.text = element_text(size = plot_params$axis_labels_size))  
...
ggplot(coeff_df %>%
  ...
  theme(plot.title = element_text(size = plot_params$title_size),
        axis.title = element_text(size = plot_params$axis_labels_size),
        axis.text = element_text(size = plot_params$axis_text_size),
        strip.text = element_text(size = plot_params$axis_labels_size))  
...
ggplot() +
  ...
  geom_segment(data = . %>%
               ...
               aes(x = Date, y = c(0.0745,0.0745,0.0745,0.0745,-0.0745,-0.0745),
                   xend = Date, yend = GaR),
               arrow = arrow(length = unit(0.1,"cm")),
               color = "blue") +
  geom_text(data = events,
            mapping = aes(x = Start + 1, y = c(0.08,0.08,-0.08),
                                          label = Event),
            color = "blue") +
  ...
  #       axis.title = element_text(size = plot_params$axis_labels_size),
  #       axis.text = element_text(size = plot_params$axis_text_size),
  #       legend.text = element_text(size = plot_params$axis_text_size))  
...
factors_df %>%
  ...
  ggtitle(paste("Factors contribution to the",
                 "forecast of the 5th percentile")) +
...
```{r plot_median_dispersion, eval=FALSE}
...
```{r plot_gdp_growth, eval=FALSE, fig.height=3}
...
\begin{itemize}
	\item
	Our results might be affected by the set of variables we used to construct the four factors.
	\item
	For robustness, we perform a sub-sampling bootstrap procedure  \citep{politis1994stationary}:
	\begin{itemize}
		\item
		We determine a minimal set of variables to be included in each group and go over all the different combinations of the other variables.
		\item
		In each iteration, we estimate a GaR model on the implied subset of variables.
\end{itemize}
...
# Negative Correlation between Median and Variance

```{r plot_median_iqr_correlations, eval=FALSE}
...
```{r plot_skew_robustness, cache=TRUE, eval=FALSE}
...
Consistent with characterization of past GDP growth
...
# Probability Integral Transform (PIT)
# Quantile R-Squared
	Quantile R-sq. \citep{giglio2016systemic} compares forecast
	performance to a constant quantile benchmark:
$$
