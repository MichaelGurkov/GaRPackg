...
geom_rect(data = events_plot,mapping = aes(xmin =start_date,xmax = end_date,
                                          ymin = -Inf,ymax = Inf),
          fill = "lightblue", alpha = 0.5) +
facet_wrap(~name, scales = "free",labeller = labeller(name = plot_names)) +
xlab(NULL) + ylab(NULL)
...
## Fin.Conditions contribute to growth in the short term, and to risks in the long term
...
```{r plot_skewness_1_quarter, fig.height=3, eval=FALSE}
...
```{r skewness_figure, out.height="60%"}
...
```{r plot_gdp_growth, fig.height=3, eval=FALSE}
...
df %>%
  ...
  # mutate(across(-date, ~scale(.,center = TRUE, scale = TRUE))) %>%
  pivot_longer(-date) %>%
...
## Goodness of fit --Quantile R-Squared \newline (the higher the better, negative means worse than intercept)
...
```{r coeff_panel_plot, fig.height=7.5}
